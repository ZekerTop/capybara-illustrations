...
这是一个角色型视觉系统。默认主体是卡皮巴拉：低矮横向的圆润身体、小圆耳靠后、眼睛小而平静、鼻梁较长、鼻口部宽钝、短爪贴近身体。它要先像卡皮巴拉，再谈松弛、可爱和幽默。它适合表达“把混乱放慢一点”“事情没有那么糟”“先做下一步”“温和但可靠”这类创作者内容。
...
- 先锁定卡皮巴拉形体：低矮桶形身体、身体大于头、小圆耳靠后、长而宽钝的鼻口部、短小爪子
...
除非用户明确说“直接生成 / 现在生图 / 不要规划”，否则先输出 shot list。文章类请求里，即使用户写“生成插图”“合理性生成插图”“为文章配图”，也默认先规划位置，不马上生图。每张图会说明：

- 放置位置或使用渠道；文章类会具体到章节、段落后或某句之后
- 画幅
- 主题
- 核心意思
- 构图模式
...
默认输出应该是带插入位置的 shot list，并在“下一步”里提示 `生成 #1` 或 `生成 #1 和 #2`。只有用户明确说“直接生成”或选择了编号后，才开始生成图片。图片生成后仍然会返回对应的插入位置和 Markdown 插入代码。
